...
# Output file path
output_file = r"e:\trae\0820焦甜香\数字化研发业务平台开发估算表_v4_精准版.xlsx"

# Data Definition
# Module, Function, Description, Frontend Days, Backend Days, AI/Algo Days
# Standard Rates: 2000 RMB/Day
# TARGET: ~2.1 Million RMB (~1050 Days total)
# STRATEGY: Dev Days ~700, Overhead adding ~50%

dev_data = [
    # 5.1 Dashboard (System Home)
    ("1. 系统首页", "工作台配置", "拖拽式自定义布局、卡片管理", 5, 3, 0),
    ("1. 系统首页", "智能聚合", "跨系统待办集成、消息中心中心", 4, 6, 0),
    ("1. 系统首页", "全局搜索", "全域检索、关联推荐", 5, 6, 3), # Reduced
    ("1. 系统首页", "KPI驾驶舱", "多维指标可视化、实时计算引擎", 6, 8, 0),

    # 5.2 Market Insight
    ("2. 市场洞察", "舆情监测系统", "全网爬虫、NLP情感识别", 8, 12, 8), # Reduced
    ("2. 市场洞察", "问卷调查平台", "可视化问卷设计、投放逻辑", 6, 8, 0),
    ("2. 市场洞察", "竞品数据库", "多维竞品档案、对标分析", 5, 6, 0),

    # 5.3 Material Management
    ("3. 原料管理", "基地数字化", "GIS地图集成、IoT环境采集", 6, 8, 0),
    ("3. 原料管理", "原料基因库", "全生命周期档案、LIMS同步", 6, 10, 0),
    ("3. 原料管理", "智能库存", "效期预警、呆滞分析", 5, 6, 2),
    ("3. 原料管理", "采购辅助决策", "需求预测算法", 5, 8, 4),

    # 5.4 Formula Management (CORE)
    ("4. 配方管理", "配方精细化维护", "结构编辑、版本回溯、成本", 12, 18, 0),
    ("4. 配方管理", "智能辅助设计", "自动寻优、替代推荐算法", 10, 15, 12), # Kept Core
    ("4. 配方管理", "三级评审流程", "多级审批流、差异比对", 5, 8, 0),

    # 5.5 Flavor Management (CORE)
    ("5. 香精香料", "香料知识库", "单体/香基档案、标准词典", 6, 8, 0),
    ("5. 香精香料", "AI智能调香", "仿香逆向、推荐模型", 10, 15, 10), # Kept Core
    ("5. 香精香料", "精密配方管理", "防差错集成、加密存储", 8, 12, 0),

    # 5.6 Auxiliary Material
    ("6. 烟用辅材", "辅材适性匹配", "透气度与焦油量预测", 5, 6, 5), # Reduced
    ("6. 烟用辅材", "在线包装设计", "3D渲染预览、批注协同", 12, 10, 0),
    ("6. 烟用辅材", "供应商协同", "资质审核、评价体系", 4, 6, 0),

    # 5.7 Process Management
    ("7. 工艺管理", "结构化工艺标准", "参数建模、版本控制", 8, 12, 0),
    ("7. 工艺管理", "工艺参数仿真", "参数模拟预测模型", 8, 10, 8),
    ("7. 工艺管理", "MES交互接口", "机台校验、参数下发", 5, 12, 0),

    # 5.8 LIMS
    ("8. 实验室管理", "全流程检测业务", "委托-检测-报告工作流", 10, 15, 0),
    ("8. 实验室管理", "仪器数据集成", "仪器IoT接口、数据解析", 5, 10, 0),
    ("8. 实验室管理", "CNAS合规", "电子记录、审计追踪", 5, 8, 0),

    # 5.9 Quality Management
    ("9. 质量管理", "移动质检", "Pad端应用、数据同步", 10, 6, 0),
    ("9. 质量管理", "缺陷视觉识别", "外观缺陷图像识别", 5, 6, 8),
    ("9. 质量管理", "QMS质量分析", "SPC控制图、质量追溯", 6, 10, 2),

    # 5.10 Online Experiment
    ("10. 在线试验", "高级排程系统", "机台冲突排程算法", 8, 10, 5),
    ("10. 在线试验", "过程实时监控", "时序数据采集", 8, 12, 0),
    ("10. 在线试验", "试验报告引擎", "多源数据聚合", 5, 8, 0),

    # 5.11 Comprehensive Mgmt
    ("11. 综合管理", "科研项目管理", "WBS、进度经费监控", 6, 10, 0),
    ("11. 综合管理", "资产效能分析", "OEE分析", 5, 6, 0),

    # 5.12 Research Assistant
    ("12. 科研助手", "科研知识图谱", "文献解析、RAG构建", 6, 8, 10),
    ("12. 科研助手", "生成式AI应用", "摘要、翻译、写作", 5, 6, 5),
]
...
# Ratios tuned to reach ~1050 days total
# Total Dev is around 660-680
# Need ~350-400 overhead days (approx 55%)

overhead_data = [
    ("项目管理 (PM)", "全生命周期", "进度、质量、风险、沟通管理", 0.12),    # 12%
    ("需求分析与架构设计", "前置阶段", "业务调研、微服务架构设计、领域建模", 0.15), # 15%
    ("系统测试与UAT", "质量保障", "单元/集成/性能/安全测试、用户验收", 0.15), # 15%
    ("部署实施与培训", "交付阶段", "多环境搭建、数据迁移、操作培训", 0.10), # 10%
]
...
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    ...
    title_cell.value = "数字化研发业务平台 - 详细开发预算表 (2.1M版)"
